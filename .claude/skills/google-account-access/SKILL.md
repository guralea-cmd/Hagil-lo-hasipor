...
- **2026-09-03:** Service account path built end-to-end and the first real tracking sheet created: **"פניות לעיתונאים"** (media outreach tracker) inside the "אוטומציה - claude" folder, with header row `כלי תקשורת | שם העורך | מייל | תאריך שליחה | ענה/לא ענה | מעקב הבא | הערות` (bold, frozen). Spreadsheet ID `1_RAVPk82GCpYgZwlUm1aIypLKgEisEObNPEKfV0PBuU`. This is presumably meant to support [[project_pr_media_plan]] outreach tracking - check that skill for the actual media target list to populate rows from, if asked to fill the sheet with real contacts.
