...
`pilates.html` is live on guralea.com with real content and a studio gallery (reformer room + chairs room), replacing the original placeholder. Leah rewrote the bio/intro text herself (used verbatim, see commit `fe2ebaf`) and picked every gallery photo by number from candidate grids Claude built and published as Artifacts. Original 15-photo set chosen across several rounds (commits `fe2ebaf`, `f3a1c68`).

**2026-09-12: added 5 more chair-room action photos** (`studio-chairs-mirror-lean.jpg`, `studio-chairs-action-front.jpg`, `studio-chairs-standing-group.jpg`, `studio-chairs-pushup-row.jpg`, `studio-chairs-bend-rear.jpg`) - Leah picked numbers 2,3,5,6,8 from a 9-photo candidate batch (source: `חומרים\WhatsApp Image 2024-12-11 at 23.42.*.jpeg`, group-class action shots not used before). Gallery now has 20 photos. If more photos are wanted later, more candidates can still be pulled from the same source folder (see raw material section below) - most of it is still unused.
...
**Both rooms confirmed by Leah (2026-08-19) to be her actual current studio** (not a former/different location) - she was direct that calling it "old" was wrong, it's "ותיק" (veteran/established), open since 2014. At the time, specific shots showing an old "פיגורא" (Figura) wall mural/banner still hanging in the chair room were skipped as a precaution.

**Correction, 2026-09-12: that precaution was wrong and should not be repeated.** Leah was confused when told some candidate photos had been excluded over this, and clarified directly: "Figura" is not some embarrassing old name to hide - people know her by both "פיגורא" and "לאה גורא", and seeing it in a photo doesn't hurt anything. **Going forward, do not exclude or filter studio photos because the old Figura signage is visible in frame** - pick photos purely on normal merits (quality, what they show, composition), same as any other candidate. She was explicit: don't search for or dwell on the Figura angle at all when presenting photo candidates - just show what fits. This is about *photos* only - it does NOT reopen the separate [[geo-seo-project]] decision to keep site *text/copy* using only "לאה גורא" branding (not "Figura") for AI-search consistency; that's a different question she has not been asked to confirm changing.
