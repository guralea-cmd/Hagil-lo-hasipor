...
**Permanent change, 2026-09-08:** the workshop was removed from the site entirely on 2026-09-07 (`workshop.html` now redirects to `index.html`, nav link and form gone - see commit "Remove workshop section from the site entirely"). Leah confirmed explicitly that **סדנה is dropped permanently from this rotation**, not just skipped for one cycle. Do not draft a workshop-type marquee line again unless she says the workshop itself is coming back.

Rotate through the three remaining content types in this fixed order, cycling: **סיפורי קהילה (עיון) → כלים ותובנות → הצטרפות לקהילה (שיתוף סיפור) → (repeat)**. Look at the last logged entry's content type to determine which type is next (older log rows may show a `סדנה` entry from before the removal - skip over those when finding the last relevant type, since that slot no longer exists).
...
**No image, ever.** Confirmed 2026-08-09, after a confused back-and-forth: the marquee strip is text-only (no image slot in its HTML), and Leah confirmed explicitly she does NOT want an image attached to this approval flow - that expectation belongs to `facebook-teaser` (headline+image+text) and the weekly blog-article skill, not this one. Don't send a file here.
