document.addEventListener("DOMContentLoaded", function () {
  ...
  // GA4 - אירוע מרכזי 1: לחיצה על כפתור הוואטסאפ (לאה, 19.9).
  // רק קישורים שפונים ללאה עצמה. קישורי "שמרי לעצמך" בבת-כמה וב-30 יום
  // הם wa.me בלי מספר, והם לא נספרים כאן.
  document.addEventListener("click", function (e) {
    var link = e.target && e.target.closest && e.target.closest('a[href*="wa.me/972506991723"]');
    if (!link || typeof gtag !== "function") return;
    gtag("event", "whatsapp_click", {
      page_path: location.pathname,
      button: link.classList.contains("wa-float-btn") ? "float" : "inline"
    });
  });

  // GA4 - אירוע מרכזי 2: קריאת סיפור - דקה בדף או גלילה של 75% (לאה, 19.9).
  // נשלח פעם אחת לכל טעינת דף, בדפי הסיפורים בלבד.
  if (/\/(story|stories)(\.html)?$/.test(location.pathname)) {
    var readSent = false;
    var sendRead = function (trigger) {
      if (readSent || typeof gtag !== "function") return;
      readSent = true;
      gtag("event", "story_read", { trigger: trigger, page_path: location.pathname });
    };
    setTimeout(function () { sendRead("time_60s"); }, 60000);
    window.addEventListener("scroll", function onScroll() {
      var doc = document.documentElement;
      var scrollable = doc.scrollHeight - window.innerHeight;
      if (scrollable <= 0) return;
      if ((doc.scrollTop || window.pageYOffset) / scrollable >= 0.75) {
        sendRead("scroll_75");
