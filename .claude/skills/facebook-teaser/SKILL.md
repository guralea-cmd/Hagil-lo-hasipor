...
2. **שי טובול** - already relaunched 2026-09-02 morning (5 destinations, old pre-badge design - his image file has since been updated with the new badge for future reuse, but his post itself is not being redone). Paid ad paused 2026-09-09.
3. **אליעזר רוה** - organic DONE 2026-09-09 (published manually after the scheduled task silently failed - see `posted-log.md` entry and `daily-open-items-report`'s new "Scheduled publish-task verification" section for the fix). Paid ad: **NEW correct-page creative `120248952673860543`, PAUSED** (rebuilt 2026-09-09 under the correct page after the page-mixup below; the original `120248950708690543` is DELETED, stale, never use it). **Pushed back to 2026-09-23** (was 9/16) - see the 2026-09-16 change below.
4. **אבי תורג'מן** - paid ad: **NEW correct-page ad `120248952672740543`, ACTIVE.** ⚠️ **Real incident, 2026-09-11, TWO separate leftover duplicates found in a full account audit:** (a) an older ad for Avi under the WRONG page (`120248889451060543`, studio page `2267713623553786`) was mistakenly reactivated by a session investigating a text-corruption false alarm, running ACTIVE alongside the correct one for hours (₪2.93 wasted) - deleted. (b) A genuinely **corrupted-text duplicate** (`120248952622470543`, correct page, but every word in the message was literally "?" characters - the real version of the shell-variable-corruption bug this file already documents) had been silently ACTIVE since the 2026-09-09 rebuild, undetected for 2 days (₪9.99 wasted) - deleted, along with a paused sibling (`120248952657170543`, ₪0 spent). **Total wasted on duplicates: ₪12.92.** `120248889451060543`, `120248952622470543`, `120248952657170543`, and page `2267713623553786` must never be used for this campaign again - the correct page for every ad here is `1190716140784281` ("הגיל הוא לא הסיפור").

**Confirmed 2026-09-11, settled - do not re-raise:** the same audit found Eliezer's (`120248952673860543`) and Shai's (`120248889500260543`) PAUSED ads still sitting in the account, plus 3 old Meta-auto-generated "boost post" campaigns from February 2026 (unrelated to this project). **Leah's explicit decision: keep all of it.** Eliezer's and Shai's paused ads are the deliberate rotation mechanism (this week's story runs, others wait paused - no cost while paused), not leftover residue - Eliezer stays ready for 2026-09-23, Shai for his next turn. The 3 old February campaigns are harmless Meta-generated clutter, cost nothing while paused - **do not attempt to delete them again**; if she ever wants them gone she'll do it herself in Ads Manager.

**Change, 2026-09-11: Wednesday 2026-09-16's organic slot is Leah's own founder story, not Avi's relaunch.** She's building both the ad and the organic post from her own on-site video (the "מתוך החיים שלי נולד החזון" video/text on `index.html`'s split-cta section, `videos/about-vision-community.mp4` - confirm this is the right one, or check `about.html` too, before building) - photo, video, and text all pulled from there, shown to her before anything goes live. Eliezer's swap (ad activation + organic relaunch) moves to **2026-09-23**, one week later than originally planned. The `community-swap-avi-to-eliezer` scheduled task (fires 2026-09-16) needs to be rewritten to reflect this before it fires - do not let it run with its old Eliezer-swap plan.

**STANDING RULE, added 2026-09-09: one story per week, and the paid ad + organic post run on a deliberate one-week offset - not the same person in both channels at once** (temporarily interrupted 2026-09-16 by Leah's own story, per above - resumes with Eliezer the following week). Every Wednesday, whichever story is currently in the **ad** gets replaced by whichever story just finished its week **organically**, and vice versa - so at any moment the ad and the organic post feature two *different* people, staggered by a week. There is no recurring automation for this - each week's swap is a fresh one-time scheduled task, since the ad side needs a specific pre-built/approved creative for whoever's coming up.

**Ad side mechanics (separate "טסט סיפורי קהילה" test campaign, ad account `act_2148850321876940`, ad set `120248718043020543` - see `facebook-ads-plan` skill):** whenever a story's turn in the ad ends, **pause its ad and verify no other ad for the same person is simultaneously active before moving on** (never delete a still-relevant one - reusable next time its organic turn comes back around; but if a stale/wrong-page duplicate is ever found, delete that one, don't just pause it - see the 2026-09-11 incident above). **Before activating a story's ad for the first time, its creative must exist and be shown to Leah for approval** (build via `POST .../adcreatives` then `POST .../ads` with `status=PAUSED`, using the same fixed short-form ad copy pattern as Shai's/Avi's/Eliezer's: hook line, 2-3 sentence facts, "אם הסיפור שלו/שלה יגרום..." line, tagline, hashtags - shorter than the organic caption, no WhatsApp line, no "לחצו" line, links straight to `stories.html#story-{docId}`, **page_id `1190716140784281`** - "הגיל הוא לא הסיפור", never the studio page `2267713623553786`). **Hard-won gotcha, 2026-09-09: build the creative's `name`/`object_story_spec` via files read with curl's `--data-urlencode field@file`, never a shell variable (`$VAR`) - Hebrew text silently corrupts to `?????` characters going through a bash variable, even though it looks fine in the terminal; caught and rebuilt once already.** Once a story has a built ad (even paused), reactivating it later is just a status flip - no need to rebuild. **Before reactivating any paused ad, check whether a duplicate for the same person already exists and is active - don't assume a clean state.**

Destinations for the organic post, both this and future relaunches (per the "REVERSED 2026-09-01" standing default): all 5 - Facebook+Instagram+TikTok for `hagil_lo_hasipor`, Facebook+Instagram for `figura_ramla`. Caption format: same structure as Shai's/Eliezer's (facts-only hook, the "אם הסיפור שלו/שלה..." line, link, slogan, hashtags, then the WhatsApp closing line with the number) - write fresh per-person facts, don't reuse another person's caption text.
...
## STANDING RULE, added 2026-09-09: every TikTok post gets its own 9:16 vertical media, never the landscape/square version used elsewhere

Confirmed via a real mistake, 2026-09-09: the first Pilates-campaign TikTok post went out using the same 4:3 landscape photo (600×450) used for Facebook/Instagram - wrong for TikTok's native vertical format. **Every image or video submitted to TikTok must be a proper 9:16 vertical (1080×1920), generated as its own separate file - never reuse the FB/IG media URL for TikTok, even after the JPEG/WEBP format fix below.** Generate via ffmpeg with a blurred-background pad (keeps the full original subject visible, never crops a person's head/feet off - a plain center-crop risks cutting them out of frame):
```
ffmpeg -y -i <source> -vf "split[bg][fg];[bg]scale=1080:1920,gblur=sigma=25[bg];[fg]scale=1080:1920:force_original_aspect_ratio=decrease[fg];[bg][fg]overlay=(W-w)/2:(H-h)/2" -frames:v 1 -q:v 3 <output.jpg>
```
(drop `-frames:v 1` and add `-c:v libx264 -pix_fmt yuv420p -an` for video clips - keep them silent per the existing audio-strip rule for b-roll). Save vertical versions under a `vertical/` subfolder next to the source (e.g. `images/pilates/vertical/`, `images/pilates/clips/vertical/`), commit, push, confirm 200 live, then use that URL - and only that URL - for the TikTok leg of the post. The FB/IG legs of the same post keep using the original (non-vertical) media as before.
