...
3. **אליעזר רוה** - organic DONE 2026-09-09 (published manually after the scheduled task silently failed - see `posted-log.md` entry and `daily-open-items-report`'s new "Scheduled publish-task verification" section for the fix). Paid ad creative built 2026-09-09 (id `120248950708690543`, PAUSED, shown to Leah for approval) - goes ACTIVE 2026-09-16 per the swap below.
4. **אבי תורג'מן** - paid ad ACTIVE 2026-09-09 (already-built ad `120248889451060543`). Organic relaunch scheduled for 2026-09-16.

**STANDING RULE, added 2026-09-09: one story per week, and the paid ad + organic post run on a deliberate one-week offset - not the same person in both channels at once.** Leah's exact structure, set 2026-09-09: every Wednesday, whichever story is currently in the **ad** gets replaced by whichever story just finished its week **organically**, and vice versa - so at any moment the ad and the organic post are featuring two *different* people, each getting both channels' exposure, just staggered by a week. Concretely, from 2026-09-09: ad = Avi, organic = Eliezer. On **Wednesday 2026-09-16**: ad flips to Eliezer (Avi's ad pauses, Eliezer's activates), organic flips to Avi (Avi's relaunch post goes out fresh to all 5 destinations). This swap is handled by the one-time scheduled task `community-swap-avi-to-eliezer` (fires 2026-09-16 09:16) - after it fires, create the next week's equivalent task by hand (there's no recurring automation for this, each week's swap is a fresh one-time task, since the ad-side needs a specific pre-built/approved creative for whoever's coming up).

**Ad side mechanics (separate "טסט סיפורי קהילה" test campaign, ad account `act_2148850321876940`, ad set `120248718043020543` - see `facebook-ads-plan` skill):** whenever a story's turn in the ad ends, pause its ad (never delete - reusable next time its organic turn comes back around) and activate the next one's. **Before activating a story's ad for the first time, its creative must exist and be shown to Leah for approval** (build via `POST .../adcreatives` then `POST .../ads` with `status=PAUSED`, using the same fixed short-form ad copy pattern as Shai's/Avi's/Eliezer's: hook line, 2-3 sentence facts, "אם הסיפור שלו/שלה יגרום..." line, tagline, hashtags - shorter than the organic caption, no WhatsApp line, no "לחצו" line, links straight to `stories.html#story-{docId}`, page_id `2267713623553786`). **Hard-won gotcha, 2026-09-09: build the creative's `name`/`object_story_spec` via files read with curl's `--data-urlencode field@file`, never a shell variable (`$VAR`) - Hebrew text silently corrupts to `?????` characters going through a bash variable, even though it looks fine in the terminal; caught and rebuilt once already.** Once a story has a built ad (even paused), reactivating it later is just a status flip - no need to rebuild.
